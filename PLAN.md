...
## 11.2 当前实现状态（2026-09-17）

已完成并通过自动测试或真实 Cursor Agent 测试：

- 常驻 ACP 连接池、排队、空闲复用和崩溃后替换。
- 每请求独立 ACP session；工具调用链独立保留并带 TTL、数量上限和客户端归属校验。
- 客户端断开时向 ACP 发送 `session/cancel`。
- `/healthz`、规范化 `/v1/models`、Responses 非流式和完整文本 SSE。
- function/custom 工具声明、MCP 转交、工具结果续传和工具 SSE 事件。
- Auto、思考档位、Fast 独立模型和不支持档位的明确错误。
- 允许根目录内的工作区校验；不覆盖 Cursor 登录所需的用户环境。
- Codex 配置备份、原子写入、官方配置完整恢复、空模型保护。
- 中文 PowerShell 交互与 ASCII `.bat` 包装、使用和换机文档。
- 6 个测试文件共 21 项测试；TypeScript 构建通过。
- 真实账号读取 223 个 Cursor 原始模型；普通响应、SSE 和 function 工具闭环均通过。

仍需在用户退出并重启 Codex Desktop 后做的外部验收：模型选择器显示、真实项目文件修改、命令提权弹窗。这些步骤涉及当前 Codex 客户端自身重启，不能在承载本任务的同一进程内自动完成。
